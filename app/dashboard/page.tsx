...
export default function HospitalDashboard() {
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full bg-[#F4F7F6] text-[#1E293B] antialiased font-sans overflow-hidden">
      ...
      <section className="flex-1 bg-white p-6 overflow-y-auto flex flex-col justify-between min-w-0">
        <div>
          ...
          {/* DATA IDENTITAS & KELUHAN UTAMA (FORM BAGIAN 1) */}
          <div className="border border-slate-200 rounded-none p-5 bg-slate-50 mb-6 flex flex-col sm:flex-row justify-between items-start gap-4 sm:gap-0">
            ...
            <div className="text-right w-full sm:w-auto border-t sm:border-t-0 sm:border-l border-slate-200 pt-3 sm:pt-0 sm:pl-6">
          ...
          {/* COMPONENT MONITOR TABEL PARAMETER TANDA VITAL (FORM BAGIAN 2) */}
          <PatientVitalsTable vitals={selectedAmb.vitals} />

          <div className="my-2" />

          {/* COMPONENT LOG CEKLIST TINDAKAN MEDIS AMBULANS (FORM BAGIAN 3) */}
        ...
        <footer className="mt-12 py-4 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-2 text-[11px] text-slate-400 font-mono">
          <div>&copy; 2026 RS Universitas Indonesia × ResQLink Platform.</div>
          <div>NODE_ID: LIVE-IGD-STATION-01</div>
        </footer>
      </section>
