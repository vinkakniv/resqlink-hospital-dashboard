...
export default function HospitalSidebar({ availableBeds, totalBeds }: HospitalSidebarProps) {
  return (
    <aside className="w-64 bg-[#192e59] text-white flex flex-col justify-between shadow-xs">
      <div>
        {/* Brand Header */}
        ...
        <nav className="p-4 space-y-1">
          <div className="text-[11px] uppercase font-bold text-white/40 px-3 mb-2 tracking-widest">Sistem IGD</div>
          <button className="w-full text-left px-4 py-2.5 bg-white/10 text-white font-semibold rounded-none text-sm flex items-center gap-3 transition-colors">
            <Activity size={16} className="text-red-500" /> Triase & Notifikasi Pra-RS
          </button>
        </nav>
      </div>

      <div className="p-4 m-4 bg-[#112244] rounded-none border border-white/5">
